...
inst_cert(){
    ...
    echo ""
    echo -e " ${GREEN}1.${PLAIN} Adobe自签证书 ${YELLOW}（默认）${PLAIN}"
    ...
    if [[ $certInput == 2 ]]; then
        ...
        green "将使用Adobe自签证书作为 Hysteria 2 的节点证书"

        cert_path="/etc/hysteria/cert.crt"
        key_path="/etc/hysteria/private.key"
        openssl ecparam -genkey -name prime256v1 -out /etc/hysteria/private.key
        openssl req -new -x509 -days 36500 -key /etc/hysteria/private.key -out /etc/hysteria/cert.crt -subj "/CN=www.adobe.com"
        chmod 777 /etc/hysteria/cert.crt
        chmod 777 /etc/hysteria/private.key
        hy_domain="www.adobe.com"
